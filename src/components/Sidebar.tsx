...
export default function Sidebar() {
  ...
  const visibleModules = modules.filter(
    (m) => canViewModule(m.id) && !disabledModules.includes(m.id),
  );
  ...
  return (
    <div
      className="w-[250px] flex flex-col h-screen sticky top-0 transition-all"
      style={{ background: CARD, borderLeft: `1px solid ${BORDER}` }}
    >
      <div
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          >
            <Settings
              className="w-4.5 h-4.5 animate-spin-slow"
              style={{ color: "#020817" }}
            />
          </div>
          <span className="text-[16px] font-[800]" style={{ color: TEXT }}>
            Factory<span style={{ color: PRIMARY }}>OS</span>
          </span>
        </div>
        <button
          onClick={toggleSidebarCollapse}
      ...
      <div className="flex-1 overflow-y-auto py-1 px-2">
            ...
            return (
              <div className="mb-1">
                ...
                <div className="space-y-px">
                  {factoryPages.map((p) => {
                    const isActive =
                      useAppStore.getState().currentCustomPageId === p.id;
                    return (
                      <button
                        key={p.id}
                        onClick={() => {
                          useAppStore.getState().setCurrentCustomPage(p.id);
                        }}
        ...
        {moduleCategories.map((cat) => {
          const catModules = visibleModules.filter(
            (m) => m.category === cat.id,
          );
