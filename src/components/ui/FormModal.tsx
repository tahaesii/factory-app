...
export interface FormField {
  name: string;
  label: string;
  type: 'text' | 'email' | 'tel' | 'number' | 'password' | 'select' | 'textarea' | 'date' | 'time' | 'checkbox' | 'radio' | 'file' | 'color';
  placeholder?: string;
  required?: boolean;
...
export default function FormModal({
  ...
  const renderField = (field: FormField) => {
    const baseInputClass = `w-full bg-card border rounded-xl px-4 py-2.5 text-primary placeholder:text-muted outline-none transition-all ${
      errors[field.name] ? 'border-red-500' : 'border-default focus:border-blue-500'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative bg-card border border-default rounded-2xl shadow-2xl w-full ${sizeClasses[size]} max-h-[90vh] overflow-hidden animate-slide-up`}>
        {/* Header */}
